...
## Quick runbook (input config + Phase 1 + Phase 2)

If you only need the core ConvergeAI execution steps, run this condensed flow:
...
python3 scripts/phase2.py --workspace ./workspaces/demo-run-01
```

You can then inspect the run artifacts:

```bash
cat ./workspaces/demo-run-01/phase1_output.json
...
Example conflict `agent_response.json` from the OpenAI adapter (showing reasoning +
future user-intervention metadata):

```json
{
  "resolved_text": "Line 2: different change from REPO B",
  "confidence": 0.7,
  "resolution": "openai",
  "reasoning": "Kept OURS because it likely represents downstream customization...",
  "user_intervention": {
    "recommended": false,
